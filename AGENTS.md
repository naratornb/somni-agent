...
**Current milestone: M4 done** (unattended reliability, all in `src/main/executor.ts`: retry-once-then-halt with `attempts` in run.json, 30-min per-task timeout SIGTERM→SIGKILL, pipeline-wide rate-limit pause with exponential backoff that does *not* burn the retry, cancellable pause, failure reasons always persisted to `task.error`, crash resume — `findOrphanedRuns`/`resumePipeline`/`abandonRun` re-run the unfinished tasks in the existing worktree — plus `powerSaveBlocker` held for the whole run. UI: orphan resume/abandon banner, ⏸ Paused chip with next-attempt time, ↻ retry marker on task chips, failure-reason banner over the log pane. Earlier: M3 pipeline — FIFO worker pool over checkbox-selected workflows, sequential-within/parallel-across bounded by concurrency (hardcoded 2 until M5), serialized `git worktree add`, unique `<ts>-<slug>` runIds; PipelineView dashboard.) Next: M5 — reports, settings (concurrency/timeout/report style, model & effort profiles), run history, worktree cleanup. Phased plan M0–M6 is in [design/architecture.md §9](design/architecture.md). When a milestone lands, update this line and the Commands section.
