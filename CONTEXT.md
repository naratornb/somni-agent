...
- **Quick Start** — the one-box golden path: describe a task on Home — typed or dictated — Groom it seeded with that text, and **Apply & run** — the Story passes the Ready gate and is queued in one act. A deliberate act, like Add to pipeline; the gate never bends for it. Dictation fills the box by default; a setting lets speech start the Groom immediately.
...
- **Handoff** — sending a session into background work ("draft this while I'm away"). The user may trigger one explicitly. M27 adds two kinds of automatic trigger, neither a turn-by-turn AI decision: an owner's groom handing off from birth is the Persona's standing instruction, chosen once and applied every time; a session's third answered Question round, and a reply that neither asks nor proposes, are universal Interview policy — fixed, documented, and firing regardless of Persona, even when none was ever picked. Navigating away is never a Handoff.
