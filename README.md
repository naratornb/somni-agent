...
somni orchestrates agent CLIs — **Claude Code** (`claude`), **Codex** (`codex`), **Gemini CLI** (`gemini`), and **Google Antigravity** (`agy`) — to run coding workflows in parallel, unattended, overnight. You define workflows of ordered tasks (each with a role/persona like *Senior Developer* or *Senior Tester*), tick the ones you want run, and hit **Run Pipeline**. Each workflow executes in its own git worktree; finished branches arrive pre-reviewed by a second AI provider — a grade and reasons waiting alongside the summary report, not just a diff. Grooming can go hands-off too: as the Project Owner persona, a groom drafts entirely in the background and lands as a Brief — a plain-language summary up top — for one **Approve & run**.
...
- **Isolation** — the app creates a dedicated worktree + `somni/<slug>-<date>` branch per workflow; review lands a grade, approved branches merge in one click, and manual merging always works.
...
3. **Create a workflow** (Workflows view). Ordered tasks, each with a title, a prompt (write it as a goal, not a diff — tasks may be retried or resumed), and a role. A good shape: Design → Implement → Test → Revise.
   ...
   - Grooming is persona-tied: as the **Technical Director** you answer up to three high-leverage questions before it proposes; as the **Project Owner** it skips the interview and drafts the whole thing in the background instead. Pick a default in Settings, or flip either the whole app or one groom at a time.
4. **Select and run** (Pipeline view). Tick the workflows to include, hit **▶ Run pipeline**. Each workflow gets a fresh worktree and `somni/<slug>-<date>` branch; tasks stream live — click a task chip to tail its output. The toolbar shows a single ⏸ Paused indicator with the retry time when a task's provider is rate-limited or auth-failed — other tasks with a usable provider keep running; Cancel stops everything.
...
6. **Morning review** (Runs & Reports view, and the Board's Review column). Per-run status, per-task durations/costs, and the report (`.somni/runs/<id>/report.md`). A finished run also carries a Branch Review grade chip — Approve / Needs Work / Reject / Ungraded, with reasons — from a second AI provider that read the Story spec and the whole diff; an approved, unmerged run shows a **Merge** button that lands its branch on whatever you have checked out in one click (refused on a dirty tree, aborted cleanly on conflict). Anything else, check out the `somni/…` branch and merge by hand, then **Clean up worktree** — somni never force-deletes a dirty worktree or unmerged branch.
7. **Tune it** (Settings view): max concurrency (keep it modest, 2–3 — overnight fan-out shares your plan's usage windows), task timeout, report style, default execution profile (pick `Auto` to walk the provider chain instead of pinning one runner). The **Merge reviewer** row pins who grades a branch before Merge appears on it (runner/model/effort, same shape as an execution profile) — leave it empty for the default cross-provider pick. The **Persona** setting (Technical Director / Project Owner) decides whether a new groom interviews you or drafts itself; it's a global default with a per-groom override in the Groom header. The **Providers panel** shows each runner's detected status/version, lets you enable/disable it and reorder the chain with ↑/↓, and set a per-provider default model/effort and a concurrency cap. Per-repo overrides go in `.somni/config.json`; per-role overrides in the role editor — a role that pins a runner overrides Auto for every task using it. The seeded **developer** role pins Claude Code for this reason: Antigravity can't read `.claude/skills/`, and developer is the role most coupled to those injected skills.
