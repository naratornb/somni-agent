...
- **Persona (M27):** who the user is to a groom — the **Technical Director**, who answers up to three Question rounds, or the **Project Owner**, who never does — decides in `sendChat` whether a message goes to a live turn or straight to a background work unit. An owner's groom routes its very first message into a work unit from birth; any session, either persona, that reaches three answered Question rounds routes its *next* message the same way — Propose Now included, so cutting the Interview short right at round three still hands off rather than getting a live reply. That post-cap routing is permanent for the session, not a one-time redirect: every message after the third answered round keeps routing to a background draft; "New chat" is the only reset back to a live turn. Resolution is item stamp → repo/global setting → the Director default, applied only at these resolution sites — the default is deliberately absent from `SETTINGS_DEFAULTS` so Home can tell a never-chosen setting from a chosen one.
