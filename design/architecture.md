...
Sidebar navigation, four destinations (M23; Playground appears in dev builds only). Groom is a routed flow step, not a nav entry; the old Pipeline and Roles destinations folded into Home and Settings.

1. **Home** (M23, default) — the front door. No repo: a single welcome hero with **Choose repo**. With a repo: the **Quick Start** box ("What do you want done overnight?") with **Suggestion chips** (`repo:suggestions` — git TODO/FIXME counts + recent commit subjects, `[]` → static fallback; chips fill the box, never submit), and beneath it the **pipeline activity**: the drain dashboard — run cards, per-subtask chips, progress bar, **Drain queue / Cancel**, **Keep Running**, drain mode/status — whose toolbar renders even with zero runs. Submitting the box grooms seeded with the text; Apply reads **Apply & run** (Stories only, never Epics) and auto-queues through the unchanged Ready gate, landing back on Home.
