...
- **Work units** (`sessions.ts`): an explicit **Handoff** ("Draft in background") runs ONE autonomous Turn under the work-unit prompt — assume-and-continue, no `somni-question` fences, Proposal spec must carry an `## Assumptions` section — via `--resume` on the transcript's session id. Cap **3** concurrent (own cap, not pipeline concurrency), FIFO queue, refused while that key's chat turn is in flight. Any outcome parks `needs-review`; Apply stays the only write path.
